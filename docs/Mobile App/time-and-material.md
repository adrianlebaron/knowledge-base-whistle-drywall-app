---
tags:
    - Admin
    - Foreman
    - Staff
    - Sub Staff
---

---

This guide provides a step-by-step walkthrough for creating a new Time and Material entry from scratch, including entering job details, adding labor and materials, and completing the setup within the app.

---

## To start, look for the side navigation menu.
From the top left corner, tap the **hamburger menu**.

![1](https://github.com/user-attachments/assets/e0467bb6-11f4-46d2-a5dd-c420e48035d2)

## Step 1: Open the Time & Material Module
Once the left menu is open, select the Time and Material module.

![2](https://github.com/user-attachments/assets/c880cb24-f318-48c2-bd52-2b18e5b7ea68)

## Step 3: T&M Module Overview
Once there, the screen will display a list of existing entries and the option to create a new one.

![3](https://github.com/user-attachments/assets/869fda68-36a1-4a63-a8b3-653496eaad39)

## Step 4: Access Dropdown Job List
Here is the interface to create a new entry. Collapse the dropdown menu by activating it.

![4](https://github.com/user-attachments/assets/035884f2-a0e3-4837-8501-5e550112cd43)

## Step 5: Job Selection
Select the job associated with the work being performed. This ensures the Time and Material entry is properly linked to the correct project.

![5](https://github.com/user-attachments/assets/0785cffc-24a5-4958-9833-f8bf1394bb1a)

## Step 6: Describe the Work Performed
Provide a brief but clear description of the specific work completed.  
This description helps project managers and office staff quickly understand the purpose of the entry.

![6](https://github.com/user-attachments/assets/252afa92-c7be-462b-9a7b-3ae76b1ae36d)

## Step 7: Define the General Scope of Work
Explain the overall task or activity performed on site.  
This field should clearly describe what work was done and how it was performed.

![7](https://github.com/user-attachments/assets/cd5069b0-c45f-4f41-85c1-8cdf02930aee)

## Step 8: Specify the Reason for the Work
Indicate why the work was required.  
Providing a reason helps justify the Time and Material work for approvals and billing.

![8](https://github.com/user-attachments/assets/e98a1946-8f14-421c-bf6b-44c37c64c841)

## Step 9: Enter the Location of the Work
Specify where on the job site the work took place.  
This is especially useful for large projects with multiple levels or zones.

![9](https://github.com/user-attachments/assets/954e560b-49bc-490a-83dc-57fcdbd7091a)

## Step 10: Select the Date of Work
Choose the date when the work was performed.  
Always ensure the date matches the actual on-site work.  
The arrows let you navigate through months. Select the desired date, then just hit **OK** to save changes.

![11](https://github.com/user-attachments/assets/d163e810-96a7-4139-8b45-b2b318c5e28d)

## Step 11: Complete Subcontractor and Authorization Details
If the work was performed on behalf of another subcontractor, complete the fields related to subcontractor responsibility and authorization.

![12](https://github.com/user-attachments/assets/3d8cd479-ed87-4030-8a2c-764a66782a75)

## Add all materials, labor, and equipment used to ensure the Time and Material entry accurately reflects the work performed and supports proper billing.

![13](https://github.com/user-attachments/assets/2538a6fa-1e28-4ce8-9de1-1acced3d1297)

## Step 12: Enter Material Details

**Name.** Enter the name of the material used. This can be written freely to match how the material is referenced on site or in billing.

**Quantity.** Enter the total amount of material used.

**Unit of Measure.** Select or type the unit used to measure the material.  
This field is flexible and can be set based on how the material is tracked, such as Ea, Box, Sheets, or similar.

![14](https://github.com/user-attachments/assets/645623d4-df91-4553-8b9d-c8e10e670bf7)

## Step 13: Enter Labor Details
Use the Labor section to record labor associated with the Time and Material work.

**Name.** Enter the type of labor performed.

**Quantity.** Enter the total amount of time worked.

**Unit of Measure.** Specify how labor is measured. This is typically entered as Hrs, but can be adjusted if needed.

![15](https://github.com/user-attachments/assets/fbf30bce-6128-40e7-b795-097b93e8a884)

## Step 14: Enter Equipment Details
If equipment was used, record it under the Equipment section.

**Name.** Enter the name of the equipment used.

**Quantity.** Enter the total time or amount the equipment was used.

**Unit of Measure.** Define how the equipment usage is measured, such as Hrs or Days.

![16](https://github.com/user-attachments/assets/117868b2-091b-4b66-b465-47040d385eb5)

## We are almost there. 
Here we can check all items we been capture and if need keep adding more. Next are two options to attach files and pictures.


![17](https://github.com/user-attachments/assets/53b87ac5-eb3b-42d9-bd9c-bf2c1da25bc3)

## Step 15: Attach Files

When clicking the cloud icon, you are taken to the file attachment interface of your own operating system.  
In this example, Android is shown. All operations may look slightly different depending on your device.

You can upload PDF files and documents. From here, you can also upload images if needed, one by one.

![18](https://github.com/user-attachments/assets/f73d20a1-e3e6-4593-a2d0-31c3ee732f05)

## Step 16: Take a Picture to Upload or Select From Your Mobile Device Library.

![19](https://github.com/user-attachments/assets/fb1d894a-0702-4591-beb3-ee45777b37bc)

## Step 17: Save T&M Entry

**Uploaded files** preview looks like this.

By clicking the bottom button action **SAVE**, you complete the process of creating a Time and Material entry from the mobile app.

**Congratulations were finish a T&M new entry, stay up to date by cheking ur documentation frequntly**
 
![20](https://github.com/user-attachments/assets/542febe6-df60-4ee7-af03-0199b387ae4c)